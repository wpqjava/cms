$(function(){
	...
	$(".cc-addTable").on("click","input[name='introPic']",function(){
		mySimpleAjax("/updateIntroPic",$(this).val());
	});
	$(".cc-addTable").on("click","input[name='spePic']",function(){
		mySimpleAjax("/updateSpePic",$(this).val());
	});
	$(".cc-addTable").on("click","input[name='showPic']",function(){
		mySimpleAjax("/updateShowPic",$(this).val());
	});
	$(".cc-addTable").on("click",".deleteAttach",function(){
		if(confirm("确认删除附件？")){
	...
	$(".cc-addTable").on("click",".insertAttach",function(){
	...
	function setAttachNode(cid){
		...
		if(cid==3){
			//首页推荐滚动大图
			$('#tr-attach').after("<tr class='temp-frame-att'><td colspan='2' class='emp'>请上传并选择一张尺寸大于1280*500的图片并勾选<主页图片>作为首页推荐滚动图片。</td></tr><tr class='temp-frame-att'><td colspan='2'>" +
			...
			//F1专题大图
			$('#tr-attach').after("<tr class='temp-frame-att'><td colspan='2' class='emp'>请上传并选择一张尺寸大于290*185的图片并勾选<专题图片>作为专题封面。</td></tr><tr class='temp-frame-att'><td colspan='2'>" +
					"<table id='ok_attach' style='text-align:center' width='890px' cellpadding='0' cellspacing='0'>" +
					"<thead><tr><Td>文件名缩略图</Td><td width='180'>文件名</td><td>文件大小</td>" +
