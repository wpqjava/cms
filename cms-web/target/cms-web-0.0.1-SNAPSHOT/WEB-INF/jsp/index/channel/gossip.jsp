...
<body>
	...
	<div id="nav">
		<div id="nav_list">
			<ul>
				...
				<li class="nav_li_first li_selected"><a href="<%=request.getContextPath() %>/gossip">围场八卦</a></li>
	...
	<div id="intro02">
		<span class="intro-span">Gossip</span>
	</div>
	<div id="intro_nav">
		...
		<c:if test="${isList eq 1 }">
			<div id="container02_inner_block">
				<c:forEach items="${datas }" var="a">
					...
					<a href="<%=request.getContextPath() %>/gossip/${a.topic.id}" class="container02_inner_p"><img width="100px" height="100px" src="<%=request.getContextPath() %>/resources/upload/thum/${a.newName}"></a>
					<a href="<%=request.getContextPath() %>/gossip/${a.topic.id}" class="container02_inner_text"><span>${a.topic.title }</span></a>
				</div>
				</c:forEach>
			</div>
		</c:if>
